...
dynamic class OGWFSConnector {
	...
	*/
	...
	/**
	Creates a getFeature request XML
	The types look like:
	types.name = layername
	types.geometryName = the attribute name of the geometry
	types.type = the type of the feature
	types.typePrefix = the prefix of the type, if there is a prefix.
	*/
	/*private function createGetFeatureRequest(version:String, types:Array, extent:Object):XML{
		var reqXml:XML= new XML();
		var reqString:String;
		reqString='<wfs:GetFeature service="WFS" version="'+version+'" ';
		reqString+='outputFormat="text/xml" xmlns:wfs="http://www.opengis.net/wfs" xmlns:ogc="http://www.opengis.net/ogc" ';
		reqString+='xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">';
		for (var i=0 ; i < types.length; i++){
			if (types[i]!=undefined){
				reqString+='<wfs:Query typeName="'+types[i].name+'">';
				//Create filter
				reqString+='<Filter><BBOX><PropertyName>'+types[i].geometryName+'</PropertyName>';
				reqString+='<gml:Box';
				if (srs!=undefined && srs.toUpperCase().indexOf('EPSG:')>0){
					reqString+=' srsName="'+srs
				}
				reqString+='>';
				reqString+='<gml:coord><gml:X>'+extent.minx+'</gml:X>';
				reqString+='<gml:Y>'+extent.miny+'</gml:Y>';
				reqString+='</gml:coord><gml:coord><gml:X>'+extent.maxx+'</gml:X>';
				reqString+='<gml:Y>'+extent.maxy+'</gml:Y></gml:coord></gml:box>'
				reqString+='</wfs:Query>';
			}
		}		
		reqString+='</wfs:GetFeature>';
		log("The getFeature String: "+reqString);
		reqXml.parseXML(reqString);
		return reqXml;
	}*/
